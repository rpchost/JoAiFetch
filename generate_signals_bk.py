...
class SignalGenerator:
    ...
    def _calculate_signal(
        ...
        if direction == SignalDirection.LONG:
            if target_price_1 <= current_close:
                return None  # predicted price is not higher → no long signal
                # else: keep target_price_1 = pred_1day.predicted_close (which is > current)
            ...
            if target_price_1 >= current_close:
                return None  # predicted price is not lower → no short signal
    ...
    async def _generate_signal_for_symbol(
        ...
        if signal is None:
           logger.debug(f"{symbol}: 'Signal rejected: predicted move direction incompatible with current price; predictions: '{predictions}. Market; {market}")
           return {'success': False, 'reason': 'Signal rejected: predicted move direction incompatible with current price'}
